...
def fetch_points_with_molmo(prompt, image_path):
    """Fetch points using the Molmo API."""
    img = Image.open(image_path)
    w, h = img.size

    input = {
        "text": f"Point to the {prompt} in the scene",
    ...
    points = extract_points(output, w, h)
    print(points)
    return points
...
def main(args: argparse.Namespace) -> None:
    ...
    if not args.image:
        if args.box is not None and args.point is None:
            ...
            seg_path = os.path.join(args.data, "seg_molmo_{}".format(args.iteration))
    else:
        if args.box is not None and args.point is None:
...
#   for name in ["train","test","novel_views"]:
    for name in ["train"]:
        ...
        for i, image in enumerate(tqdm(images, desc="Segment progress")):
            ...
            # Input point
            input_point = np.array(fetch_points_with_molmo(args.prompt, image))
            input_label = np.array([1 for i in input_point])
            prompt_embedding = sbert.encode(args.prompt, convert_to_tensor=True)
            background_embedding = sbert.encode("background", convert_to_tensor=True)
